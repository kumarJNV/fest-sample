...
export const Editfestival = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${BASE_URL}/get-festival/${id}`);
      ...
      let genobjArr = [];
      let filmkrobjArr = [];
      for (let i = 0; i < item.festival.producer.length; i++) {
        let dirOpt = {};
        dirOpt['value'] = item.festival.producer[i]._id;
        dirOpt['label'] = item.festival.producer[i].producer_name;
        prodobjArr.push(dirOpt);
      }

      for (let i = 0; i < item.festival.actor.length; i++) {
        let actOpt = {};
        actOpt['value'] = item.festival.actor[i]._id;
        actOpt['label'] = item.festival.actor[i].actor_name;
        actobjArr.push(actOpt);
      }

      for (let i = 0; i < item.festival.director.length; i++) {
        let dirOpt = {};
        dirOpt['value'] = item.festival.director[i]._id;
        dirOpt['label'] = item.festival.director[i].director_name;
        dirobjArr.push(dirOpt);
      }

      for (let i = 0; i < item.festival.genre.length; i++) {
        let genOpt = {};
        genOpt['value'] = item.festival.genre[i]._id;
        genOpt['label'] = item.festival.genre[i].genre;
        genobjArr.push(genOpt);
      }

      for (let i = 0; i < item.festival.filmmaker.length; i++) {
        let filmkrOpt = {};
        filmkrOpt['value'] = item.festival.filmmaker[i]._id;
        filmkrOpt['label'] = item.festival.filmmaker[i].name;
        filmkrobjArr.push(filmkrOpt);
      }
      //console.log(item.film.director);
      //console.log(dirobjArr);
      setTitle(item.festival.title);
      setThumbnail(item.festival.banner);
      setBanner(item.festival.thumbnail);
      setStatus(item.festival.status);
      setSrtDesc(item.festival.short_description);
      setLngDesc(item.festival.long_description);
      setRating(item.festival.rating);
      setDuration(item.festival.duration);
      setSlider(item.festival.is_slider);
      setTrailer(item.festival.trailer)
      setReceivedValue(item.festival.stream_file);
  ...
  const AddmovieData = async (e) => {
    ...
    let response = await axios.put(`${BASE_URL}/update-festival`, formData, {
      method: 'PUT',
      headers: {
    ...
    if (response.status === 200) {
      ...
      history('/view-all-festival', { state: { msg: 'Data Added successfully' } });
    }
            ...
            <div className="row mb-2">
              ...
              <div className="col-sm-6">
                <ol className="breadcrumb float-sm-right">
                  <li className="breadcrumb-item"><Link to="/view-all-festival"><i className="fa fa-eye"></i>View All festival</Link></li>
                </ol>
              </div>
            </div>
          </div>
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-6">
                        <div className="form-group">
                          ...
                          <label for="stream_title"><span className="text-danger">*</span>Festival Title</label>
                      ...
                      <div className="col-xl-6 col-lg-12 col-md-12">
                        <div className="form-group bordergaps posterGaps">
                          ...
                          <label for="tests1"><span className="text-danger">*</span>Upload Festival</label>
                          <div className="">
                            <div className="col-md-12">
                      ...
                      <div className="col-md-4">
                        <div className="form-group">
                          ...
                          <label for="duration"><span className="text-danger">*</span>Festival Duration(In Minutes)</label>
                      ...
                      <div className="col-md-4">
                        <div className="form-group">
                          ...
                          <label for="{{$strFldName}}"><span className="text-danger">*</span>Festival Rating</label>
